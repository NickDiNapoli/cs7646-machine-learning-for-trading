...
# helper function from assess_portfolio
def assess_portfolio(allocs, prices):
    # Given a df of prices, and an array of allocs, return common portfolio metrics
    start_val = 1  # arbitrary for this particular task
    normed = prices / prices.iloc[0]
    alloced = normed * allocs
    pos_vals = alloced * start_val
    port_val = pos_vals.sum(axis=1)
    # daily_rets = (alloced / alloced.shift(1)) - 1
    ...
    return cum_ret, avg_daily_ret, std_daily_ret, sr, port_val

def SR(X, prices):
    # Given a df of prices, and an array of allocs (X), return the negative Sharpe Ratio
    return -1 * assess_portfolio(X, prices)[3]
...
def optimize_portfolio(
    ...
    n = len(syms)
    allocs_guess = np.full((n,), 1/n)
    bounds = (((0,1), ) * n)
    constraints = ({'type': 'eq', 'fun': lambda inputs: 1 - np.sum(inputs)})
    min_result = spo.minimize(SR, allocs_guess, args=(prices), bounds=bounds,
